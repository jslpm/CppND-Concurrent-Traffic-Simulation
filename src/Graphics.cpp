...
void Graphics::loadBackgroundImg()
{
    ...
    // resize image
    //std::cout << "Width = " << background.cols << ", Height = " << background.rows << std::endl;
    int imageWidth = background.cols;
    int imageHeight = background.rows;
    float scaleFactor = 0.5;
    cv::resizeWindow(_windowName, (int)imageWidth*scaleFactor, (int)imageHeight*scaleFactor);
}
