...
T MessageQueue<T>::receive()
{
    ...
    std::unique_lock<std::mutex> lckReceive(_msgMtx);
    _msgCond.wait(lckReceive, [this] {return !_queue.empty(); });
...
void MessageQueue<T>::send(T &&msg)
{
    ...
    std::lock_guard<std::mutex> lckSend(_msgMtx);
    _queue.clear();
...
void TrafficLight::cycleThroughPhases()
{
    ...
    while (true)
    {
        ...
        if (timeSinceLastUpdate >= lightDelay*1000)
        {
            ...
            auto newMsg = _currentPhase;
            _messages.send(std::move(newMsg));
